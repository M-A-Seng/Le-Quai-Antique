...
<!-- nouveau plat -->
<button type="button" id="new-dish-button">Ajouter un plat</button>
<div id="new-dish-container" class="hidden">
    <form id="new-dish-form" action="/admin/<?= html($_SESSION['id']) ?>/creer/plat" method="POST" target="_self">
        ...
        <button type="button" class="close-modal" data-modal-id="new-dish-container">Annuler</button>
    </form>
</div>
...
<form action="/admin/<?= html($_SESSION['id']) ?>/modifier/plat" method="POST" target="_self">
    ...
    <div id="delete-dish" class="hidden">
        ...
        <p>Cette action est <strong>définitive</strong>. Le plat n'apparaîtra plus sur la carte du restaurant.</p>
        <button type="submit" id="delete-dish-button" name="id" formaction="/admin/<?= html($_SESSION['id']) ?>/supprimer/plat">Supprimer</button>
        <button type="button" class="close-modal" data-modal-id="delete-dish">Annuler</button>
    </div>
    ...
    <?php endif; ?>
    <ul class="dishes" data-category-id="<?= empty($rows[0]['category_id']) ? 'x' : html($rows[0]['category_id']) ?>">
        <?php foreach($rows as $row): ?>
        <li class="dish-<?= empty($row['category_id']) ? 'x' : html($row['category_id']) ?>" data-id="<?= html($row['id']) ?>">
            ...
            <span id="edit-dish-<?= html($row['id']) ?>" class="hidden">
                ...
                <label class="dish-edit-<?= html($row['id']) ?> hidden">Catégorie: 
                    <?php if (isset($categories)): ?>
                ...
                <button type="submit" name="id" value="<?= html($row['id']) ?>" class="dish-<?= html($row['id']) ?> dish-edit-<?= html($row['id']) ?> hidden" disabled>Valider</button>
                <button type="button" data-title="<?= html($row['title']) ?>" data-id="<?= html($row['id']) ?>" class="delete-dish-button dish-<?= html($row['id']) ?> dish-edit-<?= html($row['id']) ?> hidden" disabled>Supprimer</button>
            </span>
            <button type="button" class="modify-dish-button" data-id="<?= html($row['id']) ?>">Modifier</button>
        </li>
        <?php endforeach; ?>
    </ul>
    <button type="button" id="save-dish-order-<?= empty($rows[0]['category_id']) ? 'x' : html($rows[0]['category_id']) ?>" class="save-dish-order hidden" data-category-id="<?= empty($rows[0]['category_id']) ? 'x' : html($rows[0]['category_id']) ?>">Enregistrer l'ordre</button>
    <?php endforeach; ?>
</form>
<?php else: ?>
